...
async function procesarMachoteInactivo(body: AlertaRequest): Promise<string> {
  ...
  // Registrar alerta
  await supabase.from('alertas_pendientes').insert({
...
async function checkTractosOciosos(): Promise<string> {
  ...
  const { data: params } = await supabase
    ...
    .select('clave, valor')
    .in('clave', ['umbral_ocioso_aviso', 'umbral_ocioso_alerta', 'umbral_ocioso_critica', 'costo_hora_tracto'])

  const getParam = (clave: string, def: number) =>
    parseInt(params?.find(p => p.clave === clave)?.valor || String(def))

  const umbralAviso = getParam('umbral_ocioso_aviso', 4)
  const umbralAlerta = getParam('umbral_ocioso_alerta', 8)
  const umbralCritica = getParam('umbral_ocioso_critica', 12)
  ...
  const { data: tractos } = await supabase
    .from('tractos')
    .select('id, numero_economico, empresa, updated_at')
  ...
  for (const tracto of tractos || []) {
    ...
    if (horasOcioso >= umbralAviso) {
      ...
      const { data: existente } = await supabase
        ...
        .select('id')
        .eq('tipo', 'ALERTA_TRACTO_SIN_ASIGNACION')
  ...
  return `CHECK_TRACTOS_OCIOSOS: ${alertasGeneradas} alertas generadas de ${tractos?.length || 0} tractos evaluados`
}
...
async function checkUnidadesDetenidas(): Promise<string> {
  ...
  const { data: viajesActivos } = await supabase
    .from('viajes')
    .select('id, folio, tracto_id, tractos(numero_economico)')
    .in('estado', ['en_transito'])

  let alertasGeneradas = 0

  for (const viaje of viajesActivos || []) {
    const economico = (viaje as any).tractos?.numero_economico
    if (!economico) continue

    // Verificar GPS — velocidad 0 y último reporte hace más de 60 min
    const { data: gps } = await supabase
      .from('gps_tracking')
      .select('velocidad, latitud, longitud, updated_at')
      .eq('economico', economico)
      .single()

    if (!gps) continue

    const minutosDetenido = (Date.now() - new Date(gps.updated_at).getTime()) / 60000

    if (gps.velocidad === 0 && minutosDetenido > 60) {
      // Verificar si ya hay alerta activa
      const { data: existente } = await supabase
        .from('alertas_pendientes')
        .select('id')
        .eq('tipo', 'ALERTA_UNIDAD_DETENIDA')
      ...
      if (!existente?.length) {
        await procesarUnidadDetenida({
          ...
          datos: {
            economico,
            folio: viaje.folio,
            ubicacion: `${gps.latitud}, ${gps.longitud}`,
  ...
  return `CHECK_UNIDADES_DETENIDAS: ${alertasGeneradas} alertas de ${viajesActivos?.length || 0} viajes evaluados`
}
...
async function checkCitasEnRiesgo(): Promise<string> {
  ...
  const { data: viajes } = await supabase
    .from('viajes')
    .select('id, folio, eta_calculado, cita_descarga, cliente_id, clientes(razon_social)')
    .in('estado', ['en_transito', 'en_riesgo'])
    .not('eta_calculado', 'is', null)

  // Leer umbral de parametros
  const { data: param } = await supabase
    .from('parametros_sistema')
    .select('valor')
    .eq('clave', 'umbral_cita_riesgo_min')
    .single()

  const umbralMinutos = parseInt(param?.valor || '30')
  let alertasGeneradas = 0
    ...
    if (diferenciaMin > umbralMinutos) {
      ...
      if (!existente?.length) {
        await procesarCitaEnRiesgo({
          ...
          datos: {
            folio: viaje.folio,
            cliente: (viaje as any).clientes?.razon_social || 'N/A',
  ...
  return `CHECK_CITAS_EN_RIESGO: ${alertasGeneradas} alertas de ${viajes?.length || 0} viajes evaluados`
}
