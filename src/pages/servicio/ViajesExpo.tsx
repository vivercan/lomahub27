...
import { CloudUpload, Package, Clock, CheckCircle2, AlertTriangle } from 'lucide-react'
...
/* ── Component ── */
export default function ViajesExpo() {
  const [viajes, setViajes] = useState<ViajeExpo[]>([])
  ...
  const fetchViajes = useCallback(async () => {
    ...
    try {
      ...
      if (error) { console.error('Error fetching EXPO viajes:', error); return }
      setViajes(data || [])
    } catch (e) { console.error('EXPO fetch error:', e) }
  ...
  const columns = [
    ...
    { key: 'origen_ciudad', label: 'Origen', width: '130px', render: (v: ViajeExpo) => v.origen_ciudad || v.origen || '—' },
    { key: 'destino_ciudad', label: 'Destino', width: '130px', render: (v: ViajeExpo) => v.destino_ciudad || v.destino || '—' },
    { key: 'tracto', label: 'Tracto', width: '100px' },
    { key: 'kms_viaje', label: 'Km', width: '80px', render: (v: ViajeExpo) => v.kms_viaje ? v.kms_viaje.toLocaleString() : '—' },
    { key: 'estado', label: 'Estado', width: '120px', render: (v: ViajeExpo) => <Badge color={estadoColor(v.estado)}>{estadoLabel(v.estado)}</Badge> },
    { key: 'inicia_viaje', label: 'Fecha Inicio', width: '120px', render: (v: ViajeExpo) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: tokens.spacing.md, marginBottom: tokens.spacing.lg }}>
        <KPICard titulo="Total Viajes" valor={String(total)} color="primary" icono={<CloudUpload size={20} />} />
      ...
      <Card>
        <DataTable
          ...
          columns={columns}
          loading={loading}
          emptyMessage="No hay viajes de exportación en este periodo"
