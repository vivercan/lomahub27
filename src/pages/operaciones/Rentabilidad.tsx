...
export default function Rentabilidad() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch tractos for economic numbers
      ...
      // Group viajes by tracto
      const tractoGroups = new Map<string, number>()
      for (const v of (viajes || [])) {
        const tid = v.tracto_id || 'sin_tracto'
        tractoGroups.set(tid, (tractoGroups.get(tid) || 0) + 1)
      }

      // Calculate days in period for utilization
      const daysInPeriod = Math.max(1, Math.ceil((new Date(periodoFin).getTime() - new Date(periodoInicio).getTime()) / 86400000) + 1)

      const detalle: TractoDetalle[] = Array.from(tractoGroups.entries())
        .map(([tid, count]) => {
          const tracto = tractoMap.get(tid)
          return {
            tracto_id: tid,
            numero_economico: tracto?.numero_economico || tid,
            empresa: tracto?.empresa || 'N/A',
            viajes: count,
            ingresoEstimado: 0,
            costoEstimado: 0,
            margen: 0,
