...
export default function ProgramaSemanal() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const weekStart = new Date(semanaInicio + 'T00:00:00')
      const weekEnd = new Date(weekStart)
      weekEnd.setDate(weekEnd.getDate() + 6)
      const fin = weekEnd.toISOString().split('T')[0]

      // Fetch viajes in the week range
      ...
      const clienteMap = new Map((clientes || []).map(c => [c.id, c.razon_social]))

      // Fetch tractos and cajas for fleet capacity
      ...
      // Group by cliente
      const clienteGroups = new Map<string, number>()
      for (const v of (viajes || [])) {
        const cid = v.cliente_id || 'sin_cliente'
        clienteGroups.set(cid, (clienteGroups.get(cid) || 0) + 1)
      }

      const programa: ClientePrograma[] = Array.from(clienteGroups.entries()).map(([cid, count]) => ({
        cliente_id: cid,
        razon_social: clienteMap.get(cid) || 'Sin cliente',
      ...
      setData({
        ok: true,
        semana: { inicio: semanaInicio, fin },
        capacidadFlota: { tractos: totalTractos, cajas: totalCajas },
        forecastTotal,
        balance,
        alerta: balance < 0 ? `Déficit de ${Math.abs(balance)} unidades para cubrir la demanda` : null,
        programa,
      })
    } catch (err) {
